################ ALGORITHM UNMODIFIED GIBBS GLOBAL GLLiM MODEL #################
gibbs_unperturbed_kappa_xi = function(num_priorpred_samples,
  ...
  if (sampler_method_kappa_xi == "HMC") {
    source(file.path("algorithm", "hmc_kappa_xi.R"), local = TRUE)
  }
  ...
  prior_kappa_xi_param = replicate(num_priorpred_samples, sample_kappa_xi_prior())
  if(is.null(dim(prior_kappa_xi_param))) { # if kappa_xi is 1-dimensional, turn it into a matrix
    prior_kappa_xi_param = matrix(prior_kappa_xi_param, nrow = 1)
  }
  prior_ind_param = apply(prior_pop_param, MARGIN = 2, sample_ind_param)
  time_vec[1] = difftime(Sys.time(), start_time, units = "secs")
  dim_kappa_xi = dim(prior_kappa_xi_param)[1]

  prior_sim_data = sapply(
    1:num_priorpred_samples,
    FUN = function(i)
      model(prior_ind_param[, i], prior_kappa_xi_param[, i], model_param)
  )
  ...
  write_prior_pred_to_file(output_dir = output_dir,
                           ...
                           kappa_xi_param = prior_kappa_xi_param,
  ...
  prior_responses = rbind(prior_ind_param, prior_kappa_xi_param)
  ...
  kappa_xi_param_surrogate_post = samples_surrogate_post$kappa_xi_param
  ...
  for(i in 1:M){
    for (j in 1:num_surrogate_post_samples) {
      sim_data_surrogate_post[, i, j] = model(ind_param_surrogate_post[, i, j],
                                              kappa_xi_param_surrogate_post[, i, j],
                                              model_param)
    }
  }
  ...
  kappa_xi_param_surrogate_post_matrix = matrix(
    kappa_xi_param_surrogate_post,
    nrow = dim_kappa_xi,
    ncol = M * num_surrogate_post_samples
  )
  ...
  write_kappa_xi_to_csv(
    output_dir = output_dir,
    kappa_xi_param_surrogate_post_matrix,
    kappa_exists = exists("dim_kappa"),
    xi_exists = exists("dim_xi"),
    round_index = 1
  )
  ...
  responses = rbind(ind_param_surrogate_post_matrix, kappa_xi_param_surrogate_post_matrix)
  covariates = matrix(
  ...
  acceptance_rates_kappa_xi = matrix(NA, nrow = num_gibbs_samples, ncol = num_rounds - 1)

  adaptMCMC_scale_init = cov(data.frame(t(kappa_xi_param_surrogate_post_matrix)))
  ...
  # eta_matrix[, 1] = sample_eta_conjugate(ind_param_temp)
  ...
  kappa_xi_matrix = matrix(NA, nrow = dim_kappa_xi, ncol = num_gibbs_samples)
  kappa_xi_last_surrpost_sample = matrix(kappa_xi_param_surrogate_post[, , dim(kappa_xi_param_surrogate_post)[3]], ncol = M)
  kappa_xi_matrix[, 1] = apply(kappa_xi_last_surrpost_sample,
                               FUN = mean,
                               MARGIN = 1)
  
  # initialize to the mean of all surrogate posterior samples
  # kappa_xi_matrix[, 1] = apply(kappa_xi_param_surrogate_post_matrix,
  #                              FUN = mean,
  #                              MARGIN = 1)
  ...
  for (r in 2:num_rounds) {
    ...
    # run HMC warmup
    if (sampler_method_kappa_xi == "HMC") {
      if (exists("dim_kappa") && exists("dim_xi")) {
        prior_means_kappa_xi = c(prior_mean_kappa, prior_mean_xi)
        prior_sds_kappa_xi = c(prior_sd_kappa, prior_sd_xi)
      } else if (exists("dim_kappa")) {
        prior_means_kappa_xi = prior_mean_kappa
        prior_sds_kappa_xi = prior_sd_kappa
      } else {
        prior_means_kappa_xi = prior_mean_xi
        prior_sds_kappa_xi = prior_sd_xi
      }
      
      hmc_warmup = run_hmc_warmup(
        stan_mod_kappa_xi = stan_mod_kappa_xi,
        gllim_mod = gllim_lik_param,
        dim_ind_param = dim_ind_param,
        dim_kappa_xi = dim_kappa_xi,
        dim_data = dim_data,
        ind_param_init = ind_param[, , 1],
        observed_data = observed_data,
        prior_means_kappa_xi = prior_means_kappa_xi,
        prior_sds_kappa_xi = prior_sds_kappa_xi,
        init_kappa_xi = kappa_xi_matrix[, 1],
        stan_output_dir = file.path(output_dir, "stan_warmup_output"),
        iter_warmup = iter_hmc_warmup_kappa_xi,
        iter_sampling = 100
      )
    }
    ...
    for (j in 2:num_gibbs_samples) {
      ...
      if(parallelize_ind_param_MH){
        ind_MH_output = mclapply(1:M, function(i)
          sample_ind_param_MH_modified_proposal(
            ind_param_old = ind_param[, i, j - 1],
            pdf_ind = pdf_ind,
            eta = eta_matrix[, j - 1],
            kappa_xi = kappa_xi_matrix[, j - 1],
            post_mixture_param_list = reduced_post_mixture_param_list[[i]],
        ...
        for (i in 1:M) {
          ind_MH_output = sample_ind_param_MH_modified_proposal(
            ind_param_old = ind_param[, i, j - 1],
            pdf_ind = pdf_ind,
            eta = eta_matrix[, j - 1],
            kappa_xi = kappa_xi_matrix[, j - 1],
      ...
      # SAMPLE NEW KAPPA AND XI
      if (sampler_method_kappa_xi == "RAM") {
        kappa_xi_MH_output = sample_kappa_xi_adaptmcmc(
          ind_param = ind_param[, , j],
          kappa_xi_old = kappa_xi_matrix[, j - 1],
          observed_data = observed_data,
          gllim_lik_param = gllim_lik_param,
          pdf_kappa_xi_prior = pdf_kappa_xi_prior,
          scale_init = adaptMCMC_scale_init,
          burnin = burnin_kappa_xi
        )
        acceptance_rates_kappa_xi[j, r - 1] = kappa_xi_MH_output$acceptance.rate
        kappa_xi_matrix[, j] = kappa_xi_MH_output$samples[nrow(kappa_xi_MH_output$samples), ]
        
      } else if (sampler_method_kappa_xi == "HMC") {
        kappa_xi_matrix[, j] = sample_kappa_xi_warmed_up_hmc(
          kappa_xi_init = kappa_xi_matrix[, j - 1],
          ind_param_current = ind_param[, , j],
          stan_mod_kappa_xi = stan_mod_kappa_xi,
          gllim_mod = gllim_lik_param,
          num_hmc_samples = burnin_kappa_xi + 1,
          step_size = hmc_warmup$step_size,
          inv_metric_vector = hmc_warmup$inv_metric,
          metric_type = hmc_warmup$metric_type,
          stan_data = hmc_warmup$stan_data
        )
      } else {
        warning("sampler_method_kappa_xi is invalid")
      }
    ...
    write_gibbs_results_to_csv(
      ...
      acceptance_rates_kappa_xi = acceptance_rates_kappa_xi,
      round_index = r,
      param_names = param_names,
    ...
    if (r < num_rounds) {
      ...
      for (i in 1:M) {
        for (j in 1:num_gibbs_samples) {
          sim_data[, i, j] = model(ind_param[, i, j], kappa_xi_matrix[, j], model_param)
        }
      }
      ...
      kappa_xi_matrix_rep = kappa_xi_matrix[, rep(1:num_gibbs_samples, each = M)]
      new_responses = rbind(ind_param_matrix, kappa_xi_matrix_rep)
      ...
      kappa_xi_matrix[, 1] = kappa_xi_matrix[, num_gibbs_samples]
    }
