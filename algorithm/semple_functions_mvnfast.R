...
fit_gllim = function(responses,
  ...
  if (length(id_eliminate) > 0) {
    # if >0 there exist components having small mixture probabilities
    K = K - length(id_eliminate)  # reduce number of components accordingly
    warning(
      'mixprob_thresh reduced the mixture number of components from ',
      K + length(id_eliminate),
      ' to ',
      K
    )
    A = A[, , -id_eliminate]
...
compute_post_mix_prob = function(gllim_param, observed_data) {
  ...
  for (k in 1:K) {
    if (K == 1) {
      ...
      sigma = (t(Gamma_star) + Gamma_star) / 2
      post_mix_prob = pi * mvnfast::dmvn(observed_data, mu = c_star, sigma = sigma)
    }
    else{
      c_star_k = A[, , k] %*% c[, k] + b[, k]
      Gamma_star_k = Sigma[, , k] + A[, , k] %*% Gamma[, , k] %*% t(A[, , k])
      sigma_k = (t(Gamma_star_k) + Gamma_star_k) / 2
      post_mix_prob[k] = pi[k] * mvnfast::dmvn(observed_data, mu = c_star_k, sigma = sigma_k)
    }
  }
...
compute_post_param = function(gllim_param) {
  ...
  for (k in 1:K) {
    ...
    else
    {
      ...
      b_star[, k] = Sigma_star_k %*% (solve(Gamma[, , k]) %*% c[, k] - t(A[, , k]) %*%
                                        solve(Sigma[, , k]) %*% b[, k])
    }
...
sample_gmm_inflated = function(probs, means, covars, factor_cvMH = 1) {
  ...
  if (numcomp > 1)
  {
    # sigma = (t(factor_cvMH*covars[,,k]) + factor_cvMH*covars[,,k])/2
    # mvnfast::rmvn(1, mu=means[,k], sigma=sigma)
    mvtnorm::rmvnorm(1, mean = means[, k], sigma = factor_cvMH * covars[, , k])
  } else{
    # sigma = (t(factor_cvMH*covars[,,k]) + factor_cvMH*covars[,,k])/2
    # mvnfast::rmvn(1, mu=means, sigma=sigma)
    mvtnorm::rmvnorm(1, mean = means[, k], sigma = factor_cvMH * covars[, , k])
  }
}
...
likelihood_mix_prob = function(c, Gamma, pi, K, theta) {
  ...
  for (k in 1:K) {
    if (K == 1) {
      sigma = (t(Gamma) + Gamma) / 2
      likelihood_mix_prob = pi * mvnfast::dmvn(theta, mu = c, sigma = sigma)
    }
    else{
      sigma = (t(Gamma[, , k]) + Gamma[, , k]) / 2
      likelihood_mix_prob[k] = pi[k] * mvnfast::dmvn(theta, mu = c[, k], sigma =
                                                       sigma)
    }
  }
...
mixpdf = function(value, probs, means, covars) {
  ...
  if (numcomp > 1) {
    for (k in 1:numcomp) {
      sigma = (t(covars[, , k]) + covars[, , k]) / 2
      pdf = pdf + probs[k] * mvnfast::dmvn(value, mu = means[, k], sigma =
                                             sigma)
    }
  } else{
    sigma = (t(covars) + covars) / 2
    pdf = mvnfast::dmvn(value, mu = means, sigma = sigma)
  }
  return(pdf)
}