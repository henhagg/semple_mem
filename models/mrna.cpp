...
Rcpp::NumericMatrix cpp_mrna_euler_maruyama(double delta, double gamma, double k, double m0, double t0, double dt, double tt, int num_points)
{
  if (t0 > tt)
  {
...
// [[Rcpp::export]]
Rcpp::NumericVector cpp_mrna_model(NumericVector logparam, double tt, double dt, int dim_data, bool observe_time_zero)
{
  ...
  NumericMatrix sol_euler = cpp_mrna_euler_maruyama(delta, gamma, k, m0, t0, dt, tt, num_points);
  NumericVector y = log(scale * sol_euler(_, 1) + offset);
  
  NumericVector t_out;
  if(observe_time_zero) {
    t_out = linspace(0, tt, tt / (dim_data - 1));
  } else {
    t_out = linspace(tt / dim_data, tt, tt / dim_data);
  }
